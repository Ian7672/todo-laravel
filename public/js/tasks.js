...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Set minimum date untuk add dan edit
  document.getElementById('addDeadline').setAttribute('min', todayString);
  document.getElementById('editDeadline').setAttribute('min', todayString);
  
  // Set default value untuk add form
  document.getElementById('addDeadline').value = todayString;
});

// 4. Ubah fungsi toggle completed untuk sorting otomatis
document.querySelectorAll('input.toggle-completed').forEach(checkbox => {
    checkbox.addEventListener('change', function() {
      ...
      if (completed) {
        ...
        // Tambahkan badge selesai
        if (!li.querySelector('.completed-badge')) {
          const badge = document.createElement('span');
          badge.className = 'status-badge completed-badge';
          badge.textContent = 'Selesai';
          li.querySelector('.task-info').appendChild(badge);
        }
      } else {
        titleElem.classList.remove('completed');
        li.classList.remove('task-completed');
        // Hapus badge selesai
        const badge = li.querySelector('.completed-badge');
        if (badge) badge.remove();
      }

      // Sort tasks: belum selesai di atas, selesai di bawah
...
// 5. Fungsi untuk sorting berdasarkan completion status
function sortTasksByCompletion() {
  const taskList = document.getElementById('taskList');
  const tasks = Array.from(taskList.children);
  
  tasks.sort((a, b) => {
    const aCompleted = a.classList.contains('task-completed');
    const bCompleted = b.classList.contains('task-completed');
    
    // Yang belum selesai di atas (false < true)
  ...
  // Append sorted tasks back to list
  tasks.forEach(task => taskList.appendChild(task));
}

// 6. Panggil sorting saat halaman dimuat
document.addEventListener('DOMContentLoaded', function() {
  sortTasksByCompletion();
});
